...
namespace UnityGameFramework.Runtime
{
    ...
    public sealed class WebRequestComponent : GameFrameworkComponent
    {
       ...
       /// <summary>
        /// 发送 POST 请求并在请求成功后调用成功回调函数，并销毁请求对象。
        /// </summary>
        /// <param name="uri">请求的 URI。</param>
        /// <param name="json">要发送的 JSON 数据。</param>
        /// <param name="onSuccess">请求成功时调用的回调函数。</param>
        /// <returns>返回一个包含响应的 IEnumerator。</returns>
        public IEnumerator SendPostRequest(string url, string json, Action<string> onSuccess = null)
        {
            using (UnityWebRequest request =new UnityWebRequest(url, "POST"))
            {
                ...
                // 将 JSON 字符串转换为字节并设置上传处理器
                byte[] jsonToSend = new System.Text.UTF8Encoding().GetBytes(json);
                request.uploadHandler = new UploadHandlerRaw(jsonToSend);

                // 创建下载处理器
                request.downloadHandler = new DownloadHandlerBuffer();
                // request.certificateHandler =new CertificateHandler(certificateValidationCallback);
                // 发送请求
                ...
                // 请求完成，销毁请求对象
                // UnityWebRequest 会在 using 语句结束时自动 Dispose
            }
        }
        
        private bool certificateValidationCallback(byte[] certificateData, byte[] certificateAuthorityData, string url, int statusCode)
        {
            // 在这里可以添加证书验证逻辑
            // 返回 true 表示接受证书，返回 false 表示拒绝证书
            return true; // 总是返回 true 以允许所有证书
        } 
    }
}
